...
<form action="<?php echo JRoute::_('index.php?option=com_rsticketspro&view=submit'); ?>" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data" class="form-horizontal">
    <?php if ($this->params->get('show_page_heading')) { ?>
        <h1 class="font uk-h4 f900"><?php echo $this->escape($this->params->get('page_heading')); ?></h1>
    <?php } ?>
    <div class="uk-child-width-1-1 uk-child-width-1-3@s uk-grid-medium" data-uk-grid>
<?php
...
echo '<div class="">'.$this->form->getField('email')->renderField(array('class' => 'rst_email_container')).'</div>';
echo '<div class="">'.$this->form->getField('name')->renderField(array('class' => 'rst_name_container')).'</div>';
...
// message
echo '<div class="uk-width-1-1">'.$this->form->getField('message')->renderField(array('class' => 'rst_message_container')).'</div>';
...
?>
	<div>
		<button type="button" class="uk-button uk-button-primary uk-button-large uk-width-1-1 uk-box-shadow-small uk-box-shadow-hover-medium uk-border-rounded" onclick="Joomla.submitbutton('submit.save'); this.disabled = true;"><i class="fas fa-arrow-left-long"></i><?php echo JText::_('RST_SUBMIT_REQUEST'); ?></button>
	</div>
    </div>
