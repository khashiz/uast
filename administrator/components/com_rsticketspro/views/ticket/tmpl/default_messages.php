...
if (!$this->isPrint)
{
	...
	{
		echo JHtml::_('bootstrap.renderModal', 'rsticketsproHistoryModal', array(
			...
			'backdrop' => 'static'));
		?>
		<a href="#" class="btn btn-secondary" onclick="jQuery('#rsticketsproHistoryModal').modal('show');"><i class="icon-calendar"></i> <?php echo JText::_('RST_TICKET_VIEW_HISTORY'); ?></a>
		<?php
	}
	if ($this->canViewNotes)
	{
		echo JHtml::_('bootstrap.renderModal', 'rsticketsproNotesModal', array(
			...
			'backdrop' => 'static'));
		?>
		<a href="#" class="btn btn-secondary" onclick="jQuery('#rsticketsproNotesModal').modal('show');"><i class="icon-file"></i> <?php echo $this->ticket->notes ? JText::sprintf('RST_TICKET_VIEW_NOTES_NO', $this->ticket->notes) : JText::_('RST_TICKET_VIEW_NOTES'); ?></a>
		<?php
	}
	?>
		...
		<div class="media-body">
			...
			if (!$this->isPrint && !is_null($user))
			{
				...
				<div>
					...
					{
						echo JHtml::_('bootstrap.renderModal', 'rsticketsproMessageModal'.$message->id, array(
							...
							'backdrop' => 'static'));
						?>
						<a class="btn btn-secondary" onclick="jQuery('#rsticketsproMessageModal<?php echo $message->id; ?>').modal('show');" href="#"><i class="icon-edit"></i> <?php echo JText::_('RST_TICKET_EDIT_MESSAGE'); ?></a>
						<?php
					}
